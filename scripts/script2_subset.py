#!/usr/bin/env python3
"""
script2_subset.py — Lance le pipeline en SÉQUENTIEL sur une liste d'items
(ex: ton sous-ensemble stratifié de 16 items). Un item après l'autre.

Depuis le passage à follow="chain" (voir alive_common.py), chaque run est
suivi par une connexion SSE scopée à ce run précis — deux items lancés en
parallèle ne se mélangeraient plus dans le suivi comme avec l'ancienne
méthode par polling. Le séquentiel reste néanmoins conservé ici par
prudence tant que le comportement n'a pas été validé à l'échelle d'un
batch ; la parallélisation peut être reconsidérée ensuite (voir §5 du
compte rendu).

Résilience: si un item échoue (exception, timeout, réponse inattendue),
le batch continue avec le suivant — l'erreur est loggée dans le résultat
de cet item plutôt que de stopper tout le run.

Usage:
    python script2_subset.py --items-file items.txt
    # ou directement en ligne de commande:
    python script2_subset.py --item-id A --item-id B --item-id C

    items.txt: un item_id par ligne, lignes vides et commentaires (#) ignorés.

Sortie:
    - un fichier JSON par item: results/<item_id>.json
    - un résumé agrégé: results/summary.json (+ affichage console), incluant
      le coût total par item et le coût cumulé du batch.
"""
